...
// buffers.getbytes(byteRight, indexB * 4);
    ...
    public int partition(int lowIndex, int highIndex, short pivot)
        ...
        buffers.getbytes(leftRec, lowIndex * 4);
        buffers.getbytes(rightRec, highIndex * 4);
        ...
        while (lowIndex <= highIndex) {
            ...
            while ((highIndex >= lowIndex) && (toShort(rightRec,
                highIndex) >= pivot)) {
    ...
    public void quickSort(int lowIndex, int highIndex) throws IOException {
        int pivotIndex = (highIndex + lowIndex) / 2; // value used for quickSort
                                                     // comparisons
        byte[] arr = new byte[4];
        buffers.getbytes(arr, pivotIndex * 4);
...
// System.out.println(toShort(arr, pivotIndex));
        int partitionIndex = partition(lowIndex, highIndex - 1, toShort(arr,
            pivotIndex)); // beginning position is array minus pivot
        swap(partitionIndex, highIndex); // places new pivot

        if ((partitionIndex - highIndex) > 1)
